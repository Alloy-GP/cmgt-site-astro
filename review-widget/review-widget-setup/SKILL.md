...
The token is read from the `ALLOY_GITHUB_PAT` environment variable — it is **not** stored in this file. Export it in your shell before running the scripts below:
```
export ALLOY_GITHUB_PAT="github_pat_..."
```

> If this token has expired or returns a 401, ask Skyler to rotate it and update the value in your environment / secret store.
...
PAT = os.environ["ALLOY_GITHUB_PAT"]
