...
parser.add_argument("--slack-alert-url", help="Slack Incoming Webhook URL for form send-failure alerts (FORM_ALERT_SLACK_URL)", default="")
...
if NON_INTERACTIVE:
    ...
    SLACK_ALERT_URL    = ask("Slack webhook URL for form-failure alerts (same as resend-slack-alerts)")
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4b: CONFIGURE REVIEW WIDGET (src/config/review.ts + unique localStorage prefix)
# ══════════════════════════════════════════════════════════════════════════════

section("Configuring Alloy Preview Review widget")

def _page_label(stem):
    if stem == "index":
        return "Homepage"
    return stem.replace("-", " ").replace("_", " ").title()

def discover_review_items():
    """Walk src/pages for static .astro routes → [(label, path)], homepage first.
    Skips API routes and dynamic ([param]) routes — they aren't client-reviewable."""
    pages_dir = ROOT / "src" / "pages"
    items = []
    for f in sorted(pages_dir.rglob("*.astro")):
        rel = f.relative_to(pages_dir)
        if any(p == "api" for p in rel.parts) or any("[" in p for p in rel.parts):
            continue
        stem = rel.with_suffix("").as_posix()      # "index", "about", "blog/index"
        if stem == "index":
            path, label = "/", "Homepage"
        elif stem.endswith("/index"):
            base = stem[:-len("/index")]
            path, label = f"/{base}/", _page_label(base.split("/")[-1])
        else:
            path, label = f"/{stem}/", _page_label(stem.split("/")[-1])
    ...
    uniq.sort(key=lambda x: (x[1] != "/", x[0].lower()))   # homepage first, then alpha
    return uniq

review_ts = ROOT / "src" / "config" / "review.ts"
review_items = discover_review_items()
items_block = "\n".join(
    f"  {{ label: '{label}', path: '{path}', review: false }},"
    for label, path in review_items
)
review_ts.write_text(f"""// src/config/review.ts
...
""")
ok(f"src/config/review.ts — {len(review_items)} pages discovered, all review:false")

# Give the widget a unique localStorage prefix so review state never bleeds between
# client sites (the starter's xx_ default would collide across repos).
base_layout = ROOT / "src" / "layouts" / "BaseLayout.astro"
if base_layout.exists():
    bl = base_layout.read_text()
    if "xx_review_v1" in bl:
        base_layout.write_text(bl.replace("xx_review_v1", f"{SLUG}_review_v1"))
        ok(f"BaseLayout.astro — localStorage prefix → {SLUG}_review_v1")
    elif f"{SLUG}_review_v1" in bl:
        skip(f"BaseLayout.astro prefix already {SLUG}_review_v1")
    else:
        note("BaseLayout.astro — no xx_review_v1 prefix found; set the widget prefix manually")
else:
    note("BaseLayout.astro not found — skipping widget prefix")
...
env_lines = [
    ...
    "# Same Slack Incoming Webhook URL as the resend-slack-alerts project.",
    "# Leave blank to disable (forms keep working; helper just no-ops).",
    f"FORM_ALERT_SLACK_URL={SLACK_ALERT_URL or ''}",
    "",
...
if not SLACK_ALERT_URL: remaining.append(".env + Vercel — add FORM_ALERT_SLACK_URL (form-failure alerts)")
...
always_manual = [
    ...
    f"Review widget       → create Pastel project, paste link into PASTEL_BASE (src/config/review.ts)",
    f"Review widget env   → add REVIEW_SLACK_WEBHOOK + ZENDESK_* to the stg Vercel project (NOT prod)",
